...
import { setCredentails, clearCredentails, setProvider } from '../../../shared/store/actions/auth';

import { openModal, closeModal } from '../../../shared/store/actions/modal';

import { syncProfile } from '../../../shared/store/actions/profile';

import { getPlan } from '../../../shared/store/actions/plan';
...
class _Login extends Component {
  ...
  render() {
    const { isLoggedIn, picture } = this.props;

    return isLoggedIn ? (
      <a href="#" onClick={this.logout}>
        {'Logout'}
        {picture && <img className="profile-pic" src={picture} />}
      </a>
    ) : (
...
const mapStateToProps = s => ({
  ...
  picture: s.profile && s.profile.picture,
});
...
class Popup extends Component {
  ...
  login = provider => {
    const { credentials, name, profile } = provider;

    const { setCredentails, setProvider, syncProfile, getPlan, closeModal } = this.props;
